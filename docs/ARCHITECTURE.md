...
- `User`：用户名、邮箱、密码哈希、状态。
- `Role`、`UserRole`：普通用户、审核员、管理员。
- 首个管理员通过初始化命令创建，不在公开注册接口中指定角色。

### 5.2 条目与版本

- `Article` 是稳定入口，只指向当前已通过版本。
- `ArticleRevision` 保存不可变正文快照和审核状态。
- 修改提交不会直接覆盖 `Article.current_revision_id`。
- 审核通过时，在同一事务内更新版本状态和当前版本指针。
...
1. 注册时校验用户名和邮箱唯一性。
2. 密码使用成熟的密码哈希算法，不保存明文或可逆密文。
3. 登录成功后使用 HttpOnly、Secure、SameSite Cookie 保存会话或刷新凭据。
