...
- MVP 角色为管理员、审核员和贡献者。首位用户初始化为唯一管理员，管理员天然具有审核能力；后续用户默认是贡献者。
- `ReviewerApplication` 保存审核权申请及管理员决定；普通用户不能直接改变自己的角色。
- 所有权限判断通过统一的 `can_review` / `is_admin` 边界完成，不能在各路由分散硬编码。

### 5.2 条目与版本

- `Symptom` 是故障现象入口。
- `ArticleRevision` 保存正文快照、作者和审核状态。
- 每个版本记录版本号、基线版本、修改说明、修改者、审核者以及提交和审核时间。
- 修改提交不会覆盖当前公开内容。
- 审核队列同时返回待审版本和基线版本，前端把 Tiptap JSON 展开为语义块后生成可读差异。
- 审核通过时，在同一事务内将旧公开版本标记为历史版本，并发布新版本。
- 驳回保留版本与审核意见，便于提交者学习和再次修改。
- 管理员发布官方种子、撤下文章和回滚均写入 `AuditLog`；回滚复制旧内容生成新版本，不改写旧历史。
- “解决了 / 没解决”反馈绑定具体已发布版本，新版本重新计数，旧反馈仍保留。
...
审核员允许审核自己的版本。所有具有审核能力的用户共享待审队列，第一位在事务中成功完成 `pending → approved/rejected` 的操作者获胜，后续并发请求返回 `409 Conflict`。
...
- `Notification` 只保存新内容待审核、投稿通过/驳回、评论回复三类事件。
- 通知与对应业务状态变更在同一事务写入，并通过唯一幂等键防止请求重试产生重复记录。
- 前端使用 TanStack Query 共享列表与未读数缓存，主动失效并低频刷新；MVP 不使用 WebSocket 或邮件。
